...
bool __fastcall hooks::hooked_setupbones(void * ecx, void * edx, matrix3x4_t *bone_world_out, int max_bones, int bone_mask, float current_time) {
	if (ecx && ((IClientRenderable*)ecx)->GetIClientUnknown()) {
		...
		if (pNetworkable && pNetworkable->GetClientClass() && pNetworkable->GetClientClass()->m_ClassID == 35) {
			...
			g_csgo.m_globals()->m_frametime = g_csgo.m_globals()->m_interval_per_tick;			g_csgo.m_globals()->m_absoluteframetime = g_csgo.m_globals()->m_interval_per_tick * host_timescale->GetFloat();
