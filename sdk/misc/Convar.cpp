...
public:
    virtual bool RegisterConCommandBase(ConCommandBase *pVar)
    {
        ...
        g_csgo.m_cvar( )->RegisterConCommand(pVar);
        return true;
    }
};
...
{
    if(!g_csgo.m_cvar( ) || s_bRegistered)
        return;

    assert(s_nDLLIdentifier < 0);
    s_bRegistered = true;
    s_nCVarFlag = nCVarFlag;
    s_nDLLIdentifier = g_csgo.m_cvar( )->AllocateDLLIdentifier();
...
void ConVar_Unregister()
{
    ...
    assert(s_nDLLIdentifier >= 0);
	g_csgo.m_cvar( )->UnregisterConCommands(s_nDLLIdentifier);
    s_nDLLIdentifier = -1;
    s_bRegistered = false;
}
...
{
    if( g_csgo.m_cvar( ) ) {
		g_csgo.m_cvar( )->UnregisterConCommand(this);
    }
}
...
void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
{
    ...
    if( g_csgo.m_cvar( ) )
		g_csgo.m_cvar( )->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
}
...
{
	m_pOriginalCVar = g_csgo.m_cvar( )->FindVar(szCVar);
	Spoof();
}
...
SpoofedConvar::SpoofedConvar(ConVar* pCVar)
{
	...
	if (!IsSpoofed() && m_pOriginalCVar) {
		...
		//Register it
		g_csgo.m_cvar( )->RegisterConCommand(m_pDummyCVar);
...
void SpoofedConvar::Restore()
{
	if (IsSpoofed())
	{
		...
		g_csgo.m_cvar( )->UnregisterConCommand(m_pDummyCVar);
