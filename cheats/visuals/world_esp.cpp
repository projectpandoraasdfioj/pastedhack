...
void worldesp::grenades(weapon_t * m_nade, Box box) {
	...
	render::get().text(fonts[ESPNAME], box.x + (box.w / 2), box.y, color, HFONT_CENTERED_X, entity_name.c_str());
}
...
void worldesp::weapons(weapon_t * e, Box box) {
	...
	char * name = e->get_icon();

	Color color = g_cfg.esp.dropped_weapons_color;



	render::get().text(fonts[SUBTABWEAPONS], box.x + (box.w / 2), box.y, color, HFONT_CENTERED_X, name);
