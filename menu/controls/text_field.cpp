...
void C_TextField::draw() {
	...
	if (item_region.contains_point(mouse)) {
		render::get().rect_filled(n_area.x, n_area.y, n_area.w, 19, Color{ 65, 65, 65, 255 });
	}
	else
		render::get().rect_filled(n_area.x, n_area.y, n_area.w, 19, Color{ 55, 55, 55, 255 });
	...
	if (taking_input)
		render::get().text(fonts[ESPFLAG], n_area.x + 10, n_area.y + 3, Color::White, HFONT_CENTERED_NONE, to_render.c_str());
	else
		render::get().text(fonts[ESPFLAG], n_area.x + 10, n_area.y + 3, Color(220, 220, 220), HFONT_CENTERED_NONE, to_render.c_str());
}
