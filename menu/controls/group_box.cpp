...
void C_GroupBox::update_subs() {
	if (subdata.sub_tabs.size()) {
		...
		for (int i = 0; i < subdata.sub_tabs.size(); i++) {
			...
			render::get().text(fonts[ESPFLAG], position_x, area.y + 15, Color::Black, false, ">");
			render::get().text(fonts[ESPFLAG], position_x, area.y + 15, index == subdata.selected_sub ? g_cfg.menu.menu_theme[0] : Color{ 59, 59, 59, 255 }, false, ">");
...
void C_GroupBox::add_control(C_Control * control, bool first) {
	...
	if (g_cfg.menu.group_sub != -1)
		control->sub = g_cfg.menu.group_sub;
