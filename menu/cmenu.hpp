...
/*
namespace framework {
	auto checkbox = [&](std::string label, bool * state, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_CheckBox(label, state);
	};
	auto slider_int = [&](std::string label, int * value, int min, int max, std::string follower, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_SliderInt(label, value, min, max, follower, flags);
	};
	auto slider_float = [&](std::string label, float * value, float min, float max, std::string follower, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_SliderFloat(label, value, min, max, follower, flags);
	};
	auto multi_dropdown = [&](std::string label, std::vector< MultiDropdownItem_t > * items, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_MultiDropdown(label, items, flags);
	};
	auto dropdown = [&](std::string label, int * selected, std::vector< std::string > items, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_Dropdown(label, selected, items, flags);
	};
	auto dropdown_ptr = [&](std::string label, int * selected, std::vector< std::string >* items, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_Dropdown(label, selected, items, flags);
	};
	auto keybind = [&](std::string label, ButtonCode_t * key) -> void {
		new C_KeyBind(label.c_str(), key);
	};
	auto colourpicker = [&](std::string label, Color * color) -> void {
		new C_TextSeperator(label, SEPERATOR_NORMAL, { new C_ColorSelector(color) });
	};
	auto seperator = [&](std::string label, TextSeperatorType type, std::vector< C_ControlFlag * > flags = {}) -> void {
		new C_TextSeperator(label, type, flags);
	};
	auto button = [&](std::string label, std::function< void() > func = {}) -> void {
		new C_Button(label, func);
	};
	auto textbox = [&](std::string label, std::string * input) -> void {
		new C_TextField(label, input);
	};
}*/